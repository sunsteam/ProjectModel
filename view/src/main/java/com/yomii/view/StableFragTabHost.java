...
/**
 * 代码来自 {@link android.support.v4.app.FragmentTabHost}, 修改切换页面时的detach方法为hide方法，页面只会隐藏，不会移除。避免重新初始化View。
 * <p>
 * 去除 setup 方法中抛异常的代码, 用于页面编辑器预览时不报错, 但是实际使用中不能用它
 */
public class StableFragTabHost extends TabHost implements TabHost.OnTabChangeListener {
    ...
    public StableFragTabHost(Context context) {
    ...
    public StableFragTabHost(Context context, AttributeSet attrs) {
        super(context, attrs);
    ...
    public void setup() {
        ...
        //        "Must call setup() that takes a Context and FragmentManager"); mark it
        super.setup();
    }
    ...
    public void addTab(@NonNull TabSpec tabSpec, @NonNull Class<?> clss,
        ...
        if (mAttached) {
            ...
            //if (info.fragment != null && !info.fragment.isDetached()) {  mark it
            if (info.fragment != null && !info.fragment.isHidden()) {
                final FragmentTransaction ft = mFragmentManager.beginTransaction();
                //ft.detach(info.fragment); mark it
        ...
        if (mLastTab != newTab) {
            ...
            if (mLastTab != null) {
                if (mLastTab.fragment != null) {
                    //ft.detach(mLastTab.fragment);  mark it
            ...
            if (newTab != null) {
                if (newTab.fragment == null) {
                    ...
                    //ft.attach(newTab.fragment);  mark it
